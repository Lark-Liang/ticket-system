...
public interface AddressMapper {
    ...
    //插入新地址
    @Insert("INSERT INTO address (user_id, name, phone, street, address, is_default) " +
            "VALUES (#{userId}, #{name}, #{phone}, #{street} #{address}, #{isDefault})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(Address address);

    //更新地址
    @Update("UPDATE address SET name=#{name}, phone=#{phone}, street=#{street} address=#{address}, " +
